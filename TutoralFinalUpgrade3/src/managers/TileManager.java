...
public class TileManager {
    ...
    public TileManager() {
        // Load the sprite atlas from Artist.
        BufferedImage atlas = Artist.getSpriteAtlas();
        int tileSize = TILE_SIZE;
        // Calculate the number of columns and rows in the atlas.
        int cols = atlas.getWidth() / tileSize;
        int rows = atlas.getHeight() / tileSize;
        
        // Slice the atlas into individual tiles.
        for (int y = 0; y < rows; y++) {
            for (int x = 0; x < cols; x++) {
                BufferedImage subImage = atlas.getSubimage(x * tileSize, y * tileSize, tileSize, tileSize);
    ...
    /**
     * Returns a list of TileData objects representing the palette.
     * Each TileData is created using the default atlas id ("main") and its index.
     *
     * @return List of TileData objects.
     */
    public List<TileData> getPalette() {
        List<TileData> palette = new ArrayList<>();
        for (int i = 0; i < tiles.size(); i++) {
            palette.add(new TileData("main", i));
        }
        return palette;
    }
}
